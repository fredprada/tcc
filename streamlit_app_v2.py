...
if len(instancias_para_teste) == 0:
    ...
    def plot_sensor_chart(sensor, ciclo_sequencial):
        df_filtrado_sensor = X_test_pivoted_with_results[['ciclo_sequencial', 'id', sensor]].rename(columns={sensor: 'valor', 'ciclo_sequencial': 'ciclo'})
        df_filtrado_sensor = df_filtrado_sensor[df_filtrado_sensor['ciclo'] <= ciclo_sequencial]
    ...
    def incrementar_contador(seconds):
        ...
        for i in range(61):
            ...
            for j, sensor in enumerate(lista_sensores):
                with cols[j % 6]:
                    st.altair_chart(plot_sensor_chart(sensor, i), use_container_width=True)
